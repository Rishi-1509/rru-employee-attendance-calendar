...
const DB_PATH = path.join(__dirname, 'leave_calendar.db');
...
insertUser.run('admin', hash, 'System Administrator', 'admin', 'Administration', 'Admin', 'admin@university.edu');
console.log('Admin user created: admin / password123');

// Insert authority user
insertUser.run('hod_cs', hash, 'Dr. Rajesh Kumar', 'authority', 'Computer Science', 'Head of Department', 'rajesh.kumar@university.edu');
insertUser.run('hr_manager', hash, 'Mrs. Priya Sharma', 'authority', 'Human Resources', 'HR Manager', 'priya.sharma@university.edu');
...
const insertMany = db.transaction((members) => {
    for (const m of members) {
        insertUser.run(m.username, hash, m.name, 'faculty', m.dept, m.designation, `${m.username}@university.edu`);
    }
});

insertMany(facultyMembers);
...
const sampleLeaves = db.transaction(() => {
    // Some leaves for current month
    insertLeave.run(4, `${year}-${String(month + 1).padStart(2, '0')}-05`, 'casual', 'Personal work', 1);
    insertLeave.run(7, `${year}-${String(month + 1).padStart(2, '0')}-05`, 'medical', 'Doctor appointment', 1);
    insertLeave.run(12, `${year}-${String(month + 1).padStart(2, '0')}-05`, 'casual', 'Family function', 1);

    insertLeave.run(4, `${year}-${String(month + 1).padStart(2, '0')}-10`, 'earned', 'Vacation', 1);
    insertLeave.run(9, `${year}-${String(month + 1).padStart(2, '0')}-10`, 'duty', 'Conference attendance', 1);

    insertLeave.run(15, `${year}-${String(month + 1).padStart(2, '0')}-15`, 'casual', 'Personal work', 1);
    insertLeave.run(18, `${year}-${String(month + 1).padStart(2, '0')}-15`, 'medical', 'Health checkup', 1);
    insertLeave.run(6, `${year}-${String(month + 1).padStart(2, '0')}-15`, 'other', 'Workshop', 1);

    insertLeave.run(4, `${year}-${String(month + 1).padStart(2, '0')}-20`, 'casual', 'Personal work', 1);
    insertLeave.run(11, `${year}-${String(month + 1).padStart(2, '0')}-20`, 'earned', 'Family vacation', 1);
    insertLeave.run(16, `${year}-${String(month + 1).padStart(2, '0')}-20`, 'medical', 'Surgery recovery', 1);
    insertLeave.run(20, `${year}-${String(month + 1).padStart(2, '0')}-20`, 'duty', 'Exam duty at other center', 1);
});
